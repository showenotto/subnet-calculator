use dioxus::prelude::*;
use crate::common::types::{IpSubnetResult, CalculationResult};
        ...
        div { class: "overflow-x-auto mt-4",
            table { class: "w-full text-sm text-left border-collapse",
                thead { class:"bg-gray-700",
                    tr { 
                        th { class: "px-4 py-2 font-medium w-20", "ID" }
                        th { class: "px-4 py-2 font-medium", "Network" }
                        th { class: "px-4 py-2 font-medium", "Host Range" }
                        th { class: "px-4 py-2 font-medium", "Broadcast/IID" }
                    }
                }
                tbody {
                                ...
                                tr { key: "divider-{id}", class: "",
                                    td { colspan: "4", class: "px-4 py-6 text-center text-gray-500 italic",
                                        ...
                                        div { class: "text-xs mt-2", 
                                            "Showing first {first_k} and last {last_n} of {total_subnets} subnets" 
                                        }
                                    }
                                }
                            };
                            ...
                            tr { 
                                ...
                                match sub {
                                    IpSubnetResult::V4(v4) => rsx! {
                                        td { class: "px-4 py-3", "{v4.network}" }
                                        td { class: "px-4 py-3", 
                                            "{v4.first_host.as_deref().unwrap_or(\"-\")} → {v4.last_host.as_deref().unwrap_or(\"-\")}" 
                                        }
                                        td { class: "px-4 py-3", "{v4.broadcast}" }
                                    },
                                    IpSubnetResult::V6(v6) => rsx! {
                                        td { class: "px-4 py-3", "{v6.compressed}" }
                                        td { class: "px-4 py-3", "{v6.first_host} → {v6.last_host}" }
                                        td { class: "px-4 py-3", "N/A" }
                                    }
                                }
                            }
                        }
